...
direct_pairs='/home/ee577/project/training_data/Combined_merged_data.pkl'
...
with open(direct_pairs, 'rb') as f:
    X1, X2, Z = pickle.load(f) # images, features, survival

print(X1.shape, X2.shape,  Z.shape)

def normalize_survival(X1,X2, survival):
    ...
    return X1,X2 , Z_scaled

X1, X2, y = normalize_survival(X1,X2, Z)
X_train1, X_temp1, X_train2, X_temp2, y_train, y_temp = train_test_split(
    X1, X2, y, test_size=0.2, random_state=22)

# Split the temporary set into validation and test (70% validation, 30% test)
X_val1, X_test1, X_val2, X_test2, y_val, y_test = train_test_split(
    X_temp1, X_temp2, y_temp, test_size=0.7, random_state=22)

# Check the resulting shapes to verify the splits
print(f"X_train1 shape: {X_train1.shape}, X_train2 shape: {X_train2.shape}, y_train shape: {y_train.shape}")
print(f"X_val1 shape: {X_val1.shape}, X_val2 shape: {X_val2.shape}, y_val shape: {y_val.shape}")
print(f"X_test1 shape: {X_test1.shape}, X_test2 shape: {X_test2.shape}, y_test shape: {y_test.shape}")
...
def random_flip_rotate(image1,image2,  epoch):
    """
    ...
    if isinstance(image1, np.ndarray):
        image1 = torch.tensor(image1).to(device)
        image2 = torch.tensor(image2).to(device)
    ...
    if image1.dim() == 4:
        ...
        if random.random() < 0.5:
            image1 = image1.flip(3).to(device)  # Flip along the width axis
            image2 = image2.flip(3).to(device)  # Flip along the width axis
        
        # Rotate image and mask by 180 degrees with a 50% chance
        if random.random() < 0.5:
            image1 = torch.rot90(image1, k=2, dims=(2, 3)).to(device)  # Rotate by 180 degrees
            image2 = torch.rot90(image2, k=2, dims=(2, 3)).to(device)
    else:
...
class CustomDataset(Dataset):
    def __init__(self, images1, images2, labels, transform=None):
        """
        Args:
            images1 (numpy array or torch tensor): 4D tensor with shape (N, D, H, W), where N is the number of samples
            images2 (numpy array or torch tensor): 4D tensor with shape (N, D, H, W), where N is the number of samples
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(image1, np.ndarray):
            image1 = torch.tensor(image1, dtype=torch.float32).to(device)
        
        if isinstance(image2, np.ndarray):
            image2 = torch.tensor(image2, dtype=torch.float32).to(device)

        label = torch.tensor(label, dtype=torch.float32).to(device)

        # If the image is 3D (D, H, W), add channel dimension (1, D, H, W)
        if image1.ndimension() == 3:  # (D, H, W)
            image1 = image1.unsqueeze(0)  # Add channel dimension (Shape becomes: (1, D, H, W))
        
        if image2.ndimension() == 3:  # (D, H, W)
            image2 = image2.unsqueeze(0)  # Add channel dimension (Shape becomes: (1, D, H, W))
...
def train_model(
    ...
    for epoch in epoch_range:
        ...
        for batch_idx, batch in enumerate(train_loader):
            image1, image2, labels = batch['image1'], batch['image2'], batch['label']
            ...
            for i in range(image1.size(0)):
                ...
                if random.random() < 0.5:
                    img1, img2 = random_flip_rotate(img1,img2, epoch)

                # Ensure images are 4D (1, D, H, W)
                ...
                transformed_image1.append(img1)
                transformed_image2.append(img2)

            # Stack the images after ensuring all have the shape (1, D, H, W)
            transformed_image1 = torch.stack(transformed_image1).to(device)
            transformed_image2 = torch.stack(transformed_image2).to(device)
            ...
            features_1 = DTI_model(transformed_image1)  # DTI model
            features_2 = DSC_model(transformed_image2)  # DSC model

            # Forward pass through the attention mechanism in the combined model
            combined_features = torch.cat([features_1, features_2], dim=1)  # Concatenate the features

            # Now, perform the final regression prediction with a new linear layer (survival prediction)
            survival_output = combined_model.fc(combined_features)  # Survival prediction

            # Smooth L1 Loss for regression
            criterion = nn.SmoothL1Loss()
            train_loss = criterion(survival_output.squeeze(), labels)
            ...
            val_loss = evaluate_validation_loss(combined_model, val_loader, criterion, device=device)  # model in eval mode
        ...
        del image1, image2, labels, transformed_image1, transformed_image2
